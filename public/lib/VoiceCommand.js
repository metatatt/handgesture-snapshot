export class VoiceCommand {
    constructor() {
      ...
      this.phrases = ['check', 'check again','hey Joe', 'hi Joe'];
    ...
    if (this.SpeechGrammarList) {
        ...
        this.recognition.onresult = function (event) {
          var phrase = event.results[event.results.length - 1][0].transcript; // Get the latest result
            ...
            const phraseA = phrase.includes('Joe' || 'Hey')
            const phraseB = phrase.includes('check'  || 'again' || 'chuck'  || 'chat') //...pardon my pronounced
          if (phraseA) {
            const event = new Event('voiceCommandHey');
